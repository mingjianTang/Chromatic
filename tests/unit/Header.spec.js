...
describe('Header.vue', () => {
  ...
  // 测试内容：collapseChange()
  // 点击折叠侧边栏按钮，正确触发collapseChange函数
  it('click icon func is called', async () => {
  ...
  // 测试内容：collapseChange()函数执行结果测试-eventHub
  // 函数触发后，collapse取反且eventHub的$emit触发
  it('when collapseChange() is called', () => {
    const mockFn = jest.fn()
